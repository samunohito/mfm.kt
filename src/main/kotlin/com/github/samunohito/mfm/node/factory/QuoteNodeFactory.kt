...
class QuoteNodeFactory : SimpleNodeFactoryBase<MfmQuote>() {
  ...
  override fun doParse(input: String, foundInfo: SubstringFoundInfo): IFactoryResult<MfmQuote> {
    ...
    val result = FullNodeFactory().parse(contentText, contentFindResult.foundInfo)
    if (!result.success) {
      return failure()
    }
