...
class FnParser : SimpleParserBase<MfmFn>() {
  ...
  private fun sliceContent(input: String, foundInfo: SubstringFoundInfo): IParserResult<MfmNest> {
    val content = foundInfo[FnFinder.SubIndex.Content]
    return InlineParser().parse(input, content)
  }
}